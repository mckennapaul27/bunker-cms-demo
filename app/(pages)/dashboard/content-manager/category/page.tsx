...
import { categoryColumnAttributes } from '@/utils/table-colums';
...
export default async function ContentManagerPage({
    ...
    const totalCount = await Category.countDocuments(combinedQuery);
    const totalCountWithoutQuery = await Category.countDocuments();
    const data = await Category.find(combinedQuery)
        .sort(sort)
        .skip(skippage)
        .limit(pageSize);
        ...
        <DashboardWrapper>
            <StickyHeader>
                ...
                <div className={'actions'}>
                    <Link
                        href={'/dashboard/content-manager/category/create'}
            ...
            {totalCountWithoutQuery > 0 ? (
                <TableWrapper
                    ...
                    columnAttributes={categoryColumnAttributes}
                />
            ) : (
