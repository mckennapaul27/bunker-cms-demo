...
import { authorColumnAttributes } from '@/utils/table-colums';
...
export default async function ContentManagerPage({
    ...
    const totalCount = await Author.countDocuments(combinedQuery);
    const totalCountWithoutQuery = await Author.countDocuments();
    const data = await Author.find(combinedQuery)
        .sort(sort)
        .skip(skippage)
        .limit(pageSize);
        ...
        <DashboardWrapper>
            <StickyHeader>
                ...
                <div className={'actions'}>
                    <Link
                        href={'/dashboard/content-manager/author/create'}
            ...
            {totalCountWithoutQuery > 0 ? (
                <TableWrapper
                    ...
                    columnAttributes={authorColumnAttributes}
                />
            ) : (
                <EmptyState name={'Author'} />
            )}
        </DashboardWrapper>
    );
}
